...
class PgVectorProvider(VectorDBInterface):
    def __init__(self, db_client, default_vector_size: int=784,
    ...
    async def get_collection_info(self, collection_name: str) -> dict:
        async with self.db_client() as session:
            async with session.begin():
                ...
                return{
                    "table_info": table_data._asdict(),
                    "table_count": record_count.scalar()
                }
        ...
        async with self.db_client() as session:
            async with session.begin():
                check_sql = sql_text(
                    ...
                    AND pg_indexes.indexname = :index_name
                    """
                )
    ...
    async def reset_index_vector(self, collection_name: str, index_type: str=PgVectorIndexTypeEnums.HNSW.value):
        index_name = await self.default_index_name(collection_name=collection_name)
        async with self.db_client() as session:
            async with session.begin():
    ...
    async def insert_one(self, collection_name: str, txt: str, vector: list,
                    metadata: dict=None,
                    record_id: str=None):
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
    ...
    async def insert_many(self, collection_name: list, txts: list, vectors: list,
                    metadata: list=None,
        ...
        if len(metadata)==0 or not metadata:
            metadata = [None] * len(txts)
        
        async with self.db_client() as session:
            async with session.begin():
                for i in range(0, len(txts), batch_size):
                    batch_txt = txts[i: i+batch_size]
                    batch_vectors = vectors[i: i+batch_size]
                    batch_metadata = metadata[i: i+batch_size]
                    batch_record_ids = record_ids[i: i+batch_size]
                    values = []

                    for _txt, _vector, _metadata, _record_id in zip(batch_txt, batch_vectors, batch_metadata, batch_record_ids):
